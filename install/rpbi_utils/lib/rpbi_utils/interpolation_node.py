...
import rpbi_utils.src.rpbi_utils.interpolation as interpol_lib
from ros_pybullet_interface_sub.src.rpbi.custom_ros_tools import load_config
...
class TrajManager:
    ...
    def popFirstTrajElem(self):
        ...
        if self.motion_interp_plan.shape[1] == 0:
            self.node.get_logger().error("All the trajectory data has been consumed")
            return None
...
class ROSTrajInterface(object):
    ...
    def __init__(self):
        ...
        # Name of node
        super().__init__('ros_traj_interface')
        self.name = self.get_name()
        # Initialization message
        self.get_logger().info("%s: Initializing class", self.name)

        # Setup constants
        consuming_freq = self.declare_parameter('~consuming_freq', 100)
        self.dt = 1.0/float(consuming_freq)

        # safety check
        if consuming_freq > 200:
            self.get_logger().infoo("%s: Shutting down to high FREQUENCY of consuming in node ", self.name)
            self.clean_shut_down()

        # Get ros parameters
        self.traj_config_file_name = self.declare_parameter('~traj_config')

        # if interpolation is for a robot
        namespace = self.declare_parameter('~namespace', '')

        # stream the interpolated data or not
        self.declare_parameter('/stream_interpolated_motion_flag', True)

        #  traj_manager
        self.setup_traj_manager(self.traj_config_file_name, namespace)

        # Establish connection with planning node
        self.get_logger().info(f"{self.name}: Waiting for self.current_traj_topic topic")
        msgTraj = self.wait_for_message(self.current_traj_topic, Float64MultiArray)
    ...
    def publish_next_waypt_to_ROS(self, event):
        ...
        if not self.get_parameter('/stream_interpolated_motion_flag'):
            return
    ...
    def clean_shut_down(self):
        print('')
        self.get_logger().info("%s: Shutting down interpolation node ", self.name)
...
def main(args=None):
    ...
    node = ROSTrajInterface()
    rclpy.spin(node)
    node.clean_shut_down()
    node.destroy_node()
    rclpy.shutdown()
