...
class HardwareInterface(Node):
    def __init__(self, node, link):
        ...
        #self.kit = ServoKit(channels=16) #Defining a new set of servos using the Adafruit ServoKit Library
        ...
        """ 'servo_multipliers' and 'complementary_angle' both work to flip some angles, x, to (180-x) so that movement on each leg is consistent despite
            physical motor orientation changes """
        ...
        """ 'physical_calibration_offsets' are the angle required for the servo to be at their 'zero' locations. These zero locations
            are NOT the angles defined in the IK, but rather locations that allow practical usage of the servo's 180 degree range of motion. 

            - Offsets for HIP servos align the hip so that the leg is perfectly vertical at an input of zero degrees, direct from the IK.
            - Offsets for UPPER leg servos map align the servo so that it is horizontal toward the back of the robot at an input of zero degrees, direct from the IK. 
            - Offsets for LOWER leg servos map align the servo so that it is vertically down at zero degrees. Note that IK requires a transformation of
                angle_sent_to_servo = (180-angle_from_IK) + 90 degrees to map to this physical servo location.  """
    ...
    def set_actuator_positions(self, joint_angles):
        ...
        and offsets for complementary angles.
        ...
        for leg_index in range(4):
            for axis_index in range(3):
                try:
                    ...
                    self.get_logger().warn(f"Warning - I2C IO error: {e}")
    ...
    def joint_angles_to_servo_angles(self, joint_angles):
        ...
        for leg in range(4):
            ...
            THETA0 = lower_leg_angle_to_servo_angle(self.link, m.pi/2-THETA2, THETA3 + np.pi/2) # TODO draw a diagram to describe this transformation from IK frame to LINK analysis frame
...
def calculate_4_bar(th2, a, b, c, d):
    """Using 'Freudenstein's method', it finds all the angles within a 4 bar linkage with vertices ABCD and known link lengths a, b, c, d
...
def lower_leg_angle_to_servo_angle(link, THETA2, THETA3):
    ...
    link: Leg_linkage
        ...
        the physical design of the link
...
def impose_physical_limits(desired_joint_angles):
    ...
    Returns
    -------
    possible_joint_angles: numpy array 3x4 of float angles (radians)
        The angles that will be attempted to be implemented, limited to a possible range
    '''
