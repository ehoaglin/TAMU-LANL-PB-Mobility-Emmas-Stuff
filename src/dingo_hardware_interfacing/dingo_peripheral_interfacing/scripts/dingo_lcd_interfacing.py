...
class DingoDisplayNode:
    def __init__(self):
        super().__init__('dingo_display_node') #changed from rospy.init_node('dingo_display_node') to super().__init__('dingo_display_node')
        self.rate = self.create_rate(50)  # 50 Hz #changed from rospy.Rate(50) to self.rate = self.create_rate(50)
        ...
        self.battery_voltage_subscriber = self.create_subscription("/electrical_measurements", ElectricalMeasurements, self.update_battery_percentage) #changed from rospy.Subscriber() to self.create_subscription()
    ...
    def run(self):
        try:
            ...
            except Exception as e: # changed rospy.ROSInterruptException to Exception
                self.get_logger().error(str(e)) # changed rospy.logger() to self.get_logger().error()
                self.ssid = "N/A"
            ...
            except Exception as e: # changed from rospy.ROSInterruptException to Exception
                #rospy.logerr(str(e))
                #self.ipAddress = '-:-:-:-'
    ...
    def loop(self):
        ...
        while rclpy.ok():
            ...
            self.get_logger().info("Quitting...") # changed from rospy.loginfo() to self.get_logger().info()
            self.disp.clear()
            self.disp.module_exit()


if __name__ == '__main__':
    rclpy.init() #added this
    node = DingoDisplayNode()
    node.get_logger().info("Display node started, ouputting to display") # changed from rospy.loginfo() to node.get_logger().info()
    node.loop()
