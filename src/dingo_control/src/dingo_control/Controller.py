...
from dingo_utilities.src.dingo_utilities.Utilities import clipped_first_order_filter
...
class Controller:
    ...
    def __init__(
        ...
        rclpy.init() # initialo\ize rclpy
        self.node = rclpy.create_node('node 1') # adding this

                ################# ROS PUBLISHER FOR TASK SPACE GOALS ##############
        self.task_space_pub = self.node.create_publisher('task_space_goals', TaskSpace, queue_size=10) # (2) changing to self.node = rclpy.node.create_publisher() (1)replacing rospy.Publisher with rclpy.create_publisher
        self.joint_space_pub = self.node.create_publisher('joint_space_goals', JointSpace, queue_size=10) #  (2) changing to self.node = rclpy.node.create_publisher() (1)replacing rospy.Publisher with rclpy.create_publisher
    ...
    def run(self, state, command):
        ...
        if previous_state != state.behavior_state:
            logger = rclpy.logging.get_logger('my_logger') # adding this line to bypass node creation
            logger.info("State changed from %s to %s", str(previous_state), str(state.behavior_state)) #changing from rospy.loginfo() to logger.info()
