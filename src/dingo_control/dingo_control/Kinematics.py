...
class Kinematics:
    ...
    def leg_explicit_inverse_kinematics(self, r_body_foot, leg_index):
        ...
        a_2 = asin(sin(self.config.phi)*self.config.L1/len_A)
        a_3 = pi - a_2 - self.config.phi               
        ...
        offset = np.array([0.0,self.config.L1*cos(theta_1),self.config.L1*sin(theta_1)])
        translated_frame = r_body_foot_ - offset
        
        if is_right: R2 = theta_1 + self.config.phi - pi/2
        else: R2 = -(pi/2 - self.config.phi + theta_1) #This line may need to be adjusted
        R2 = theta_1 + self.config.phi - pi/2
    ...
    def forward_kinematics(self, angles, is_right = 0):
        ...
        """
        x = self.config.L3*sin(angles[1]+angles[2]) - self.config.L2*cos(angles[1])
        y = 0.5*self.config.L2*cos(angles[0]+angles[1]) - self.config.L1*cos(angles[0]+(403*pi)/4500) - 0.5*self.config.L2*cos(angles[0]-angles[1]) - self.config.L3*cos(angles[1]+angles[2])*sin(angles[0])
        z = 0.5*self.config.L2*sin(angles[0]-angles[1]) + self.config.L1*sin(angles[0]+(403*pi)/4500) - 0.5*self.config.L2*sin(angles[0]+angles[1]) - self.config.L3*cos(angles[1]+angles[2])*cos(angles[0])
        if not is_right:
            y = -y
        return np.array([x,y,z])

    def angle_corrector(self, angles=[0,0,0]):
